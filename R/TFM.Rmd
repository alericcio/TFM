...
hist(frecuencias[, 'Frecuencia'], xlim=c(0,1000), xlab = "Frecuencias", ylab = "Número de veces que se repiten")
...
```{r include=TRUE}
# agrupo todos los pacientes que han tenido los GRD más frecuentes, mostrando también el DCO.
...
Los pacientes analizados son 35285. Los CDM son 18, 81 GRD y 2190 DCO. Creo tablas de frecuencias para las variables categóricas cómo edad, género, periodo temporal.

```{r include=TRUE}
# Agrupo los valores y calculo la frecuencia absoluta
diag_frecuentes$Edad <- cut(diag_frecuentes$Edad, breaks = c(0, 6, 12, 19, 27, 60, 108), labels = c("primera infancia 0-5", "infancia 6-11", "adolescencia 12-18","juventud 19-26", "adultez 27-59", "vejez 60 y mas"), right = FALSE)

table1 <- table(diag_frecuentes$Gender, diag_frecuentes$Edad)
#labels = c("primera infancia 0-5", "infancia", "adolesciencia","juventud", "adultez", "vejez") ,
# Muestro la tabla de contingencia con la frecuencia absoluta de cada valor de la variable "edad"
# La moda es el valor con mayor frecuencia absoluta

summary(table1)

#Muestro la tabla de frecuencias completa redondeando las frecuencias relativas a 3 decimales
transform(tablaedad,
          FreqAc = cumsum(Freq),
